...
def now_central():
    """Get current time in Central Time (no UTC)."""
...
def create_empty_workbook():
    ...
    today = now_central().strftime("%m_%d_%y")
    ...
    # Send path immediately to Flask server
    try:
        resp = requests.post("http://localhost:8000/set_excel_file_path", json={"excel_file_path": excel_file_path})
        if resp.status_code == 200:
            print("[+] Excel path sent to Flask server.")
        else:
            print(f"[!] Failed to send Excel path: {resp.text}")
    except Exception as e:
        print(f"[!] Exception sending Excel path: {e}")

def sort_excel_by_timestamp():
    global excel_file_path
    if not excel_file_path:
        print("No Excel file path set. Cannot sort.")
        return

    wb = load_workbook(excel_file_path)
    ws = wb.active

    # Find all rows with data (skip empty rows)
    data_rows = []
    # Start from row 1 to max_row
    for row in range(1, ws.max_row + 1):
        # Read A, B, C, D
        trial_val = ws[f"A{row}"].value
        msg = ws[f"B{row}"].value
        ts = ws[f"C{row}"].value
        delay = ws[f"D{row}"].value

        # Skip rows with trial header only (like "TRIAL 1" in col A and rest blank)
        if trial_val and isinstance(trial_val, str) and trial_val.startswith("TRIAL"):
            continue

        # Only consider rows that have timestamp
        if ts:
            ...
            data_rows.append((row, trial_val, msg, ts_dt, delay))

    # Sort by timestamp (4th element in tuple)
    data_rows.sort(key=lambda x: x[3])

    # Now rewrite the sorted data back to the sheet starting from row 1 or row 2
    # Since you want to keep trial headers separate, we just overwrite all data starting row 1, ignoring trial headers.
    # Alternatively, clear the sheet and rewrite without trial headers.

    # Clear all rows except first (if you want to keep header row) or clear all.
    ws.delete_rows(1, ws.max_row)

    # Write sorted data back starting row 1, columns B-D (because column A had trial info)
    start_row = 1
    for i, (_, trial_val, msg, ts_dt, delay) in enumerate(data_rows):
        r = start_row + i
        # col A leave blank, as trial labels won't make sense after sorting globally
        ws[f"A{r}"] = None
        ws[f"B{r}"] = msg
        # Write timestamp as string with microseconds
        ws[f"C{r}"] = ts_dt.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]  # millisecond precision
        ws[f"D{r}"] = delay

    wb.save(excel_file_path)

def write_trial_header(trial_num):
    global excel_file_path
    ...
    wb = load_workbook(excel_file_path)
    ws = wb.active
    ws.append([f"TRIAL {trial_num}"])  # Only trial header, no extras
    wb.save(excel_file_path)
...
def save_to_excel():
    ...
    try:
        resp = requests.post("http://localhost:8000/set_excel_file_path", json={"excel_file_path": excel_file_path})
        if resp.status_code == 200:
            print("[+] Excel path set on Flask server successfully.")
        else:
            print(f"[!] Failed to set Excel path: {resp.text}")
    except Exception as e:
        print(f"[!] Exception sending Excel path to Flask: {e}")
    wb.save(excel_file_path)
    session_data.clear()
...
def on_start():
    ...
    if current_trial >= trial_count:
        ...
        return
    current_trial += 1
    write_trial_header(current_trial)
    start_button.config(state='disabled')

    sync_windows_time()
    buffer_seconds = 5
    start_time = now_central() + timedelta(seconds=buffer_seconds)
...
def on_closing():
    try:
        sort_excel_by_timestamp()
        messagebox.showinfo("Done", "All data has been written and sorted in the spreadsheet.")
    except Exception as e:
        messagebox.showerror("Error", f"An error occurred: {e}")
    root.destroy()  # or your GUI main window variable
...
# === GUI ===
def launch_gui():
    global root, start_button, video_log
